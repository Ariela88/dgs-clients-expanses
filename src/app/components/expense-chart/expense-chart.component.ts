...
export class ExpenseChartComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      (!changes['updateChartTrigger']?.firstChange || !changes['selectedCategory']?.firstChange)
    ) {
      this.generateChart();
    }
  }
  ...
  generateChart() {
    if (this.client) {
      ...
      const labels: string[] = Object.keys(categoriesData).flatMap(category =>
        Object.keys(categoriesData[category])
      ).sort();

      const totalExpenses = labels.map(date =>
        this.client!.expenses
          .filter(
            expense => expense.type === this.selectedCategory || this.selectedCategory === 'All'
          )
          .filter(
            expense =>
      ...
      const datasets: ChartDataset[] = Object.keys(categoriesData).map(category => {
  ...
  getDistinctCategories(): string[] {
    return this.client ? Array.from(new Set(this.client.expenses.map(expense => expense.type))) : [];
  }
}
